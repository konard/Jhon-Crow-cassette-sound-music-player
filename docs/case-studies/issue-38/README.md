...
## Phase 4: Session Data Folder Issue (2026-01-24)

### Problem Report

**05:02:51 UTC** - Owner reports: "не работает, теперь рядом с exe создаётся папка с доп информацией (Local Storage и тп)" (doesn't work, now folders with additional info are created next to the exe: Local Storage, etc.)

**Issue**: While the first fix correctly set the `userData` path, Electron was still creating additional folders next to the executable, such as:
- `Local Storage/`
- `Session Storage/`
- `GPUCache/`
- `Cache/`

### Root Cause: Missing sessionData Configuration

The first fix only addressed `userData` but failed to configure `sessionData`. According to Electron documentation:

**userData** stores app configuration files (settings.json, etc.)
**sessionData** stores data generated by web pages, including:
- localStorage
- cookies
- disk cache
...
When `sessionData` is not explicitly set, Electron creates these folders using its default paths, which in portable mode ends up being next to the executable.

### Research Findings

From Electron documentation:

> The directory for storing data generated by Session, such as localStorage, cookies, disk cache, downloaded dictionaries, network state, DevTools files. By default this points to userData. Chromium may write very large disk cache here, so if your app doesn't rely on browser storage like localStorage or cookies, it is recommended to set this directory to other locations to avoid polluting the userData directory.

Key finding: **sessionData must be set BEFORE the 'ready' event is emitted**.

### The Complete Solution

Both `userData` AND `sessionData` must be set to the same portable folder:
...
if (isPortable) {
  ...
  try {
    ...
    // Set BOTH paths to prevent additional folders
    app.setPath('userData', portableUserData);
    app.setPath('sessionData', portableUserData);  // NEW: Prevents Local Storage, etc.
...
**Files Changed**:
- `src/main.js:8-29` - Added `sessionData` path configuration
- `experiments/test-portable-session-data.js` - Comprehensive test for session data storage

### Expected Behavior After Fix

**BEFORE FIX** (After first iteration):
```
cassette-music-player.exe
UserData/
  settings.json
Local Storage/        ← Created next to EXE (BAD)
Session Storage/      ← Created next to EXE (BAD)
GPUCache/             ← Created next to EXE (BAD)
```
...
UserData/
  settings.json
  Local Storage/      ← Inside UserData (GOOD)
  Session Storage/    ← Inside UserData (GOOD)
  GPUCache/           ← Inside UserData (GOOD)
  Cache/              ← Inside UserData (GOOD)
```

### Available Electron Path Names

For complete portable mode support, here are all settable paths:

**Essential for Portable Mode**:
- `userData` - App configuration files
- `sessionData` - Session data (localStorage, cookies, cache)

**Optional (can be set if needed)**:
- `appData` - Per-user application data directory
- `cache` - Cache folder
- `logs` - Log files directory
- `crashDumps` - Crash dumps directory

**Read-only paths** (cannot be set):
...
Created comprehensive test: `experiments/test-portable-session-data.js`

This experiment verifies:
1. Both `userData` and `sessionData` are set correctly
2. Both paths point to the same folder
3. No additional folders created next to executable
4. All Electron storage contained within UserData/

### Updated References

Additional research sources consulted:
- [Electron session API Documentation](https://www.electronjs.org/docs/latest/api/session)
- [GitHub Issue: Feature Request for sessionData path](https://github.com/electron/electron/issues/28122)
- [GitHub Issue: AppData folder still created after setting userData](https://github.com/electron/electron/issues/24536)
- [Electron app.setPath() Documentation](https://www.electronjs.org/docs/latest/api/app)

### Status

✅ Implementation complete - both userData and sessionData configured
⏳ CI verification in progress
⏳ Awaiting user testing to confirm no additional folders are created
